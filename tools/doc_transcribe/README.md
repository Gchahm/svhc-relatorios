...
These schemas stay within the Anthropic **structured-output subset**: objects/arrays/enums/`anyOf`/`$ref` with `additionalProperties:false`, but **no** numeric/length/pattern bounds or recursion. So every schema MUST stay within that subset: no `minimum`/`maximum`/`min*`/`max*`/`pattern`/`format`/`if`/`then`/`else`/`not`/`allOf`/`oneOf`, every object declares `additionalProperties:false`, every `$ref` is a local `#/$defs/<name>` and the `$defs` graph is acyclic (no recursion). `tests/test_subset_constraints.py` enforces this automatically over every registered schema. (Note: the `api` backend no longer sends a schema as a wire `output_config` — that structured-output grammar caps optional params at 24, which `danfe`/`nfse` exceed — so it embeds the schema in the prompt and validates above the backend, like `cli`. The subset is still kept: it is the exact grammar the stdlib `validator.py` implements, and keeps the schemas unambiguous + portable.)
