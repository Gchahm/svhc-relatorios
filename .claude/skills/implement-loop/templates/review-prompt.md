...
Invoke the `pr-review` skill with `{{PR_NUMBER}}` and follow it exactly: skip if this head was
already reviewed, otherwise post ONE review with inline comments and a verdict (`REQUEST_CHANGES`, or
`APPROVE` / `VERDICT: approve` with `commit_id` at the head you reviewed — load-bearing for the merge
gate).
