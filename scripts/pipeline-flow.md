...
- All Python ↔ Cloudflare access goes through **`scripts/common/d1.py`**, a thin wrapper around the `wrangler` CLI (`d1 execute`, `r2 object put/get`). The `--remote` flag selects **production** Cloudflare; default is **local Miniflare**.
- **Two local databases (feature 061 / issue #107).** Locally there are two isolated Miniflare state dirs: **staging** = wrangler's default `.wrangler/state` (used by every normal command — `dev`, `preview`, `db:migrate:dev`, `db:studio:dev`, and any manual `python -m scraper/analysis …` run) and **test** = `.wrangler/state-test` (used by the automated test/seed/e2e tooling, which wipes + reseeds freely). The switch is one env var, **`SVHC_WRANGLER_PERSIST`**: when set, `d1.py` appends `--persist-to <dir>` to every **local** wrangler call (relative values resolve against the repo root); unset ⇒ staging default. It is **never** applied to `--remote`. The test entrypoints (`test:py:integration`, `e2e:seed`, `e2e:smoke`, `test:e2e`) set it to `.wrangler/state-test`, and the e2e preview server forwards `--persist-to` so the served Worker reads the same test DB. The suites **self-bootstrap** the test DB (the harness/seed apply migrations on first run); `pnpm db:migrate:test` migrates it explicitly and `pnpm preview:test` serves it. A manual pipeline run outside the test scripts hits **staging** unless you export `SVHC_WRANGLER_PERSIST` yourself.
